...
build $target_image=image_name $tag=default_tag:
    ...
    # Pass build-args from env vars if set
    if [[ -n "${TEAM_ID:-}" ]]; then BUILD_ARGS+=(--build-arg TEAM_ID="$TEAM_ID"); fi
    if [[ -n "${GROUP_IDS:-}" ]]; then BUILD_ARGS+=(--build-arg GROUP_IDS="$GROUP_IDS"); fi
    if [[ -n "${LEVEL:-}" ]]; then BUILD_ARGS+=(--build-arg LEVEL="$LEVEL"); fi
    if [[ -n "${AGENT_URL:-}" ]]; then BUILD_ARGS+=(--build-arg AGENT_URL="$AGENT_URL"); fi
    if [[ -n "${SYSCHECK_URL:-}" ]]; then BUILD_ARGS+=(--build-arg SYSCHECK_URL="$SYSCHECK_URL"); fi

    # This actually builds the image!
    PODMAN_BUILD_ARGS=(${BUILD_ARGS[@]+"${BUILD_ARGS[@]}"} ${LABELS[@]+"${LABELS[@]}"} --pull=newer --tag "${target_image}:${tag}" --file Containerfile)
